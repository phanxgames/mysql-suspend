...
/**
 * @class
 * <p>For use with MySQL databases.</p>
 ...
 * </pre>
 * <p>Note: the dictionaryjs, mysql and suspend modules should be automatically installed as dependencies, but you may need to install these as well.</p>
 *
 ...
 * {@link MysqlSuspend#setResume} method, before the suspend.resume functionality will
 * be enabled.</p>
 *
...
class MysqlSuspend {
     ...
     *     var db = new MysqlSuspend(suspend.resume,myconfig);
     *     //..
     * </pre>
    ...
    usePool() {
        if (dbConfig.config.usePool==true) return true;
        return false;
    }
     ...
     * <p>Alias: open(cb)</p>
     * @param {MysqlSuspend~cbOnError} [cb=null] - Use callback or leave null to use suspend.resume.
     *                                 <br> Returns cb()
     */
    start(cb) {
        var self = this;
        ...
        try {
            ...
            if (self.usePool()) {
                ...
                if (self.pool==null)
                    self.pool = mysql.createPool(dbConfig.config.mysql);
     ...
     * <p>Alias: close(cb)</p>
     * @param {MysqlSuspend~cbOnEmpty} [cb=null] - Use callback or leave null to use suspend.resume.
     *                            <br>Returns cb(err)
     */
    end(cb) {
        var self = this;
     ...
     * @param {MysqlSuspend~cbOnQuery} [cb=null] - Use callback or leave null to use suspend.resume.
    ...
    query(sql, paras, cb) {
        ...
        //TODO implement parametrized queries

        //Start timer to collect query execution time
        var querystart = timeStart();

        //execute the query on the mysql client
     ...
     * <p>Try using instead the {@link MysqlSuspend#mergeHelper} to generate the
     ...
     * @param {MysqlSuspend~cbOnMerge} [cb=null] - Use callback or leave null to use suspend.resume.
     ...
     *                      <br>You may also use {@link MysqlSuspend#whereHelper}
     ...
     *                      <br>You may also use {@link MysqlSuspend#orderByHelper}
     ...
     * See {@link MysqlSuspend#merge} for more information.</p>
     * <h4>Example:</h4>
     ...
     * See {@link MysqlSuspend#selectHelper}, {@link MysqlSuspend#deleteHelper},
     * {@link MysqlSuspend#updateHelper}, {@link MysqlSuspend#mergeHelper}.
     * <h4>Example:</h4>
     * <pre>
     *     //..
     *     db.whereHelper({
     ...
     * See {@link MysqlSuspend#selectHelper}.
     ...
     *  //..
     * </pre>
     * @param {MysqlSuspend~cbAsyncForEachIterator} cbIterator - Returns cb(index,item,cbNext).
     *              <br>Must call cbNext() to continue.
     * @param {MysqlSuspend~cbAsyncForEachFinal} [cbFinal=null] - Returns cb().
 ...
 * @callback MysqlSuspend~cbOnEmpty
 */

/**
 * @callback MysqlSuspend~cbOnError
 ...
 *     ]
 * </pre>
 * @callback MysqlSuspend~cbOnQuery
 ...
 * @callback MysqlSuspend~cbOnMerge
 * @param {Error} err - An error if there was one, or null.
